...
$(document).ready(function () {
    //#region time-dependent greeting
    document.querySelector("#aptGreeting").innerHTML = new Date().getHours() >= 12 ? 'こんにちは' : 'おはよう' + '！ ビヌダです。';
    document.querySelector("#aptGreeting").innerHTML = new Date().getHours() >= 18 ? 'こんばんは' : 'こんにちは' + '！ ビヌダです。';
    //#endregion
    ...
    // List of sentences
    var _CONTENT = ["様々な興味ぶかい物について話すつもり", "しかし、毎日書かないかもしれないけど", "がんばって！"];
    ...
    .then(res => res.json())
    .then(data => {
        // skidz({text: data.text, source: 'en', target: 'ja'}).then((quote) => {
        //     document.querySelector("#quote").innerHTML = `"${quote}"`;
        // }),
        // skidz({text: data.author, source: 'en', target: 'ja'}).then((author) => {
